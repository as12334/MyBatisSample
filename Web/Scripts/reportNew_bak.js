define(function (require, exports, module) {
    ...
    function setLottery(v) {
        ...
        if (v == '1') {
            ...
            for (var key in kcData) {
                lotteryHtml += '<option value="' + key + '">' + kcData[key] + '</option>';
            }
            sltLottery.html(lotteryHtml);
            setSelect(sltLottery.val());
            type_list.show();
    ...
    function setSelect(v) {
        ...
        if (v == '') {
            ...
            $("#phasebylottery").prop('disabled', true);
            $("#DateRadio").prop('checked', true);
            return;
        } else {
            
            if (v=='100' ||( $("#txtbegintime").val() == todaytime && $("#txtendtime").val() == todaytime)) {
                $("#phasebylotteryRadio").prop('disabled', false);
                $("#phasebylottery").prop('disabled', false)
            }
            else {
                $("#phasebylotteryRadio").prop('disabled', true);
                $("#phasebylottery").prop('disabled', true);
    ...
    $("#btnSearch").click(function () {
        ...
        if ($('input:radio[name="t_FT"]:checked').val() == '0' && $("#phasebylottery").val() == '')
        {          
            top.setDialogBox('&nbsp;&nbsp;&nbsp;請選擇要查詢的獎期!!&nbsp;&nbsp;&nbsp; ', function () { $("#phasebylottery").focus();});
            return false;
        }
        if (sltLottery.val() == '100') {
            src = 'ReportList_' + radioVal + '_six.aspx';
        } else {
            src = 'ReportList_' + radioVal + '_kcNew.aspx';
        }
